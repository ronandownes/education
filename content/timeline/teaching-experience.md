---
layout: doc
permalink: /timeline.html
title: Timeline
eyebrow: TIMELINE
intro: A reverse-chronological working record of schools, roles, subjects, SEN/AEN experience and the interview evidence each setting gives me.
---
## Coláiste Dún Iascaigh, Cahir — Current Interview Focus (2026) {:#current-interview-focus}
**Key line: This is a prospective Science & SEN role, so use it as a comparison point with my actual experience rather than as employment history.**

### Why this role overlaps strongly with my experience
- **ETB / community-college context:** I can compare the setting with experience in Thomond Community College and Nenagh College.
- **Science:** I have taught Junior Cycle Science as well as Mathematics, Physics-related material and practical STEM subjects.
- **SEN / AEN:** I have worked in learning support, withdrawal, mixed-needs mainstream classes, collaborative settings and with students requiring differentiated access.
- **Mixed programmes and pathways:** my experience includes Junior Cycle, Transition Year, Leaving Certificate Higher and Ordinary Level, Foundation Level and LCA.
- **Classroom structure:** I can draw on experience across very different school settings when discussing routines, low-level disruption, challenging behaviour and use of the school code.
- **Inclusion and relationships:** I can connect SEN practice with accessibility, regulation, student voice, restorative follow-up and wider pastoral structures.

### Useful comparison prompts
- What feels familiar here because of my ETB/community-college experience?
- Which Science examples best transfer into a Science & SEN post?
- Which learning-support examples show that I can adapt without lowering expectations?
- Which examples show collaboration with AEN staff, SNAs, year heads and management?
- What would I need to learn quickly about this school's own systems, supports and expectations?

### Interview areas this can connect to
**Teaching & Learning · Classroom Management · SEN / AEN & Inclusion · Differentiation & Accessibility · Assessment · Planning & Curriculum · Relationships & Wellbeing · Professional Responsibility**

## Presentation Secondary School, Thurles — 2025–26 {:#presentation-2025-26}
**Key line: Recent experience across Junior Cycle, TY, Leaving Certificate and LCA gives me current evidence of planning, differentiation, assessment and classroom practice.**

### Teaching
- Junior Cycle Mathematics
- Transition Year Mathematics
- Leaving Certificate Ordinary Level Mathematics
- LCA Mathematical Applications
- Pastoral / tutor-type responsibility during the week

### Evidence to capture here
- strongest lesson or sequence
- mixed-ability differentiation
- reluctant learner / confidence-building example
- assessment and feedback example
- classroom-management example
- parent / colleague / year-head collaboration
- contribution beyond the timetable

### Interview areas
**Teaching & Learning · Classroom Management · Differentiation · Assessment · Planning · Relationships**

## Nenagh College — 2024–25 {:#nenagh-2024-25}
**Key line: Use this entry to capture experience in an ETB setting and compare it directly with Cahir.**

### Evidence to build out
- subjects and classes taught
- levels and programmes
- SEN / learning-support involvement
- school routines and code-of-behaviour experience
- collaboration with colleagues and management
- examples that transfer directly to Coláiste Dún Iascaigh

### Cahir overlap
This is one of the most useful comparison points for the current Cahir interview because both sit within an ETB context. Keep notes here on what was similar in structure, expectations, student support and professional responsibility, while avoiding assumptions that the two schools operate identically.

## Thomond Community College — 2022–24 {:#thomond-2022-24}
**Key line: This is a major evidence bank because it combines Mathematics, Science, Coding/Digital Media, varied programmes and significant mixed-needs experience.**

### Teaching and programmes
- Junior Cycle Mathematics
- Leaving Certificate Mathematics
- Junior Cycle Science
- Junior Cycle Coding
- Digital Media Literacy
- LCA / Foundation / TY contexts
- learning-support and mixed-needs work

### Strong evidence areas
- teaching across a wide range of attainment
- Science and practical/STEM teaching
- technology-rich teaching
- differentiation and accessibility
- SEN / AEN collaboration
- classroom management across different groups
- assessment and reporting
- contribution to whole-school initiatives

### Cahir overlap
This is probably the strongest existing comparison point for a Science & SEN role: a community-college context, Junior Cycle Science, varied student need, multiple programmes and the need to balance structure, inclusion and high expectations.

## Scoil Mhuire agus Íde, Newcastle West — 2021–22
**Key line: Strong evidence for Mathematics across year groups together with learning-support experience.**

### Teaching
- Junior Cycle Mathematics
- Transition Year Mathematics
- Leaving Certificate Ordinary Level Mathematics
- learning support

### Interview evidence
Capture examples of withdrawal/support work, differentiation, concrete or visual approaches, rebuilding confidence and collaboration with support structures.

## John the Baptist Community School, Hospital — 2021
**Key line: Add the specific classes, responsibilities and strongest examples from this period as the timeline is completed.**

Use this entry for evidence that is distinct from the later ETB and voluntary-secondary settings.

## Earlier Teaching Experience
Build backwards from here through the remaining schools on the CV. For each school keep the same compact structure:

- **Dates and school type / ethos**
- **Subjects, classes and levels**
- **SEN / AEN or learning-support work**
- **Roles and contribution**
- **One or two strong interview stories**
- **What I learned**
- **Which of the eight interview areas it supports**

The timeline is a working evidence bank, not simply a CV. Add detail only when it helps me remember what I actually did, what I learned, and how that experience answers an interview question.
